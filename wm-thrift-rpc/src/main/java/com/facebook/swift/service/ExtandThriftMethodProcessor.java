...
/**
 * Desc: ExtendThriftMethodProcessor
 *
 * @author wangmin
 * Date: 2021/3/1
 * Time: 2:27 下午
 */
@ThreadSafe
public class ExtandThriftMethodProcessor {
    ...
    private final List<ConsumeExpand> consumeExpandList;

    public ExtandThriftMethodProcessor(
            Object service,
            String serviceName,
            ThriftMethodMetadata methodMetadata,
            ThriftCodecManager codecManager,
            List<ConsumeExpand> consumeExpandList
    ) {
        this.service = service;
        this.serviceName = serviceName;
        this.consumeExpandList = consumeExpandList;
                    ...
                    if (!oneway) {
                        ...
                        if (exceptionCodec != null) {
                            ...
                            TApplicationException applicationException =
                                    ExtendThriftServiceProcessor.createAndWriteApplicationException(
                                            out,
                                            requestContext,
                                            method.getName(),
                                            sequenceId,
    ...
    private ListenableFuture<?> invokeMethod(Object[] args) {
        try {
            if (!CollectionUtils.isEmpty(consumeExpandList)) {
                consumeExpandList.forEach(ConsumeExpand::consume);
            }
    ...
    private <T> void writeResponse(TProtocol out,
        ...
        if ("exception".equals(responseFieldName) && result instanceof Throwable) {
            if (TraceConsumeTProtocol.SERVER_LOCAL_ACTIVE_SPAN.get() != null) {
                ...
                SpanDecorator.onError((Throwable) result, TraceConsumeTProtocol.SERVER_LOCAL_ACTIVE_SPAN.get());
            }
        }
